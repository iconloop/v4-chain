...
#
	...
	# Market: USDT-USD
	dasel put -t json -f "$GENESIS" '.app_state.prices.market_params.[]' -v "{}"
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[2].pair' -v 'USDT-USD'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[2].id' -v '1000000'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[2].exponent' -v '-9'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[2].min_price_change_ppm' -v '1000'  # 0.100%
	dasel put -t json -f "$GENESIS" '.app_state.prices.market_prices.[]' -v "{}"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[2].id' -v '1000000'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[2].exponent' -v '-9'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[2].price' -v '1000000000'          # $1 = 1 USDT.
...
#	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[36].price' -v '8695478191'          # $0.008695
	...
	dasel put -t int -f "$GENESIS" '.app_state.rewards.params.market_id' -v '1'
